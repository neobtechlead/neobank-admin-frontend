...
import NavBar from "@/app/overview/components/NavBar";
import {usePathname} from "next/navigation";
import {navBarItems, profileMenuItems} from "@/utils/constants";
import OverViewTitle from "@/app/overview/components/OverViewTitle";
import HeaderTitleWithBody from "@/app/overview/components/HeaderTitleWithBody";
...
const Header = () => {
    ...
    const showHeadingTitle = () => {
        ...
        for (const {href, label, description} of navBarItems) {
            if (pathName === href) {
                // Check if the path is "/overview" to display a specific component
                return pathName === '/overview' ? <OverViewTitle/> :
