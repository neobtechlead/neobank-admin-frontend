...
import type {APIResponse, PaginatedResponse, Users} from "@/utils/types/dto";
...
const useGetUsersData = (page = 0, size = 10) => {
    return useQuery({
        ...
        queryFn: async () => {
            const response = await http.get<APIResponse<PaginatedResponse<Users>>>(`${BASE_URL}/merchants/users`, {
                params: {page, size}
            })
